...
static int tw68_s_crop(struct file *file, void *f, const struct v4l2_crop *crop)
{
	...
	if (rect.top < b->top)
		rect.top = b->top;
	if (rect.top > b->top + b->height)
		rect.top = b->top + b->height;
	if (rect.height > b->top - rect.top + b->height)
		rect.height = b->top - rect.top + b->height;

	if (rect.left < b->left)
		rect.left = b->left;
	if (rect.left > b->left + b->width)
		rect.left = b->left + b->width;
	if (rect.width > b->left - rect.left + b->width)
		rect.width = b->left - rect.left + b->width;

	dprintk(DBG_FLOW, "%s: setting cropping rectangle: top=%d, left=%d, "
		    "width=%d, height=%d\n", __func__, rect.top,
		    rect.left, rect.width, rect.height);
	dev->crop_current = rect;
	return 0;
}
#endif
