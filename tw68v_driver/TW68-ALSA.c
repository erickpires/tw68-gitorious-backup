	...
	if (audio_irq)
	{
		...
		for(k =0; k<8; k++)
		{
			if (audio_irq & (1<<k))
			{
				...
				/*
				printk_n(" TW68_alsa_irq  Audio CH:%d   aPB: %X   RUNTIME HWptr 0x%p, 0x%X, jf 0x%x Pdz 0x%x 0x%x  BFz 0x%x  DMA  %X  %X  %X\n",
						k,  ((audio_PB >>k)& 0x1), 
				...
				*/
				...
				printk_n( "TW68_alsa_irq:   card 0x%p,  substream 0x%p, runtime BD 0%d  hw_ptr 0x%p, runtime->control->appl_ptr 0x%p,  cap avail 0x%X\n",
...
static int snd_card_TW68_capture_trigger(struct snd_pcm_substream * substream, int cmd)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_capture_trigger\n");
	dev = card_TW68 ->dev;



	if (!card_TW68) {
		printk_n(KERN_ERR "BUG:  snd_card_TW68_capture_trigger TW68 can't find device struct.\n");
		return -ENODEV;
	}

	avail = snd_pcm_capture_avail(runtime);

	//printk_n( "snd_card_TW68_capture_trigger:   card 0x%p,   runtime BD 0%d  hw_ptr 0x%p, runtime->control->appl_ptr 0x%p,  Cap avail xfer_align 0x%X\n",
	...
	printk_n(  "%s()   cmd %X  ret %X   DMA_CHANNEL_ENABLE 0x%X   0x%X\n", __func__, cmd, ret, dwReg, dwRegF);
	return ret;
...
static int snd_card_TW68_capture_prepare(struct snd_pcm_substream * substream)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_capture_prepare\n");
	if (!card_TW68) {
		printk_n(KERN_ERR "BUG:  snd_card_TW68_capture_prepare TW68 can't find device struct.\n");
		return -ENODEV;
	}
	dev = card_TW68 ->dev;
	...
	printk_n("      AUDIO_CTRL2: 0x%x         0x%X\n", dwREGA, dwREG);

	printk_n( "%s() dev %p  substream->runtime->rate %d  dwREG: 0x%X\n", __func__, dev, Currenrt_SAMPLE_RATE, dwREG);
	return 0;
}
...
snd_card_TW68_capture_pointer(struct snd_pcm_substream * substream)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_capture_pointer\n");
	//snd_card_TW68_t *card_TW68 = snd_pcm_substream_chip(substream);
...
static void snd_card_TW68_runtime_free(struct snd_pcm_runtime *runtime)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_runtime_free\n");
	printk_n( "%s()\n", __func__);
				      ...
				      struct snd_pcm_hw_params * hw_params)
{
	...
	int err =0;
#endif
	printk_tmp("CALLED [alsa]: snd_card_TW68_hw_params\n");
	printk_n( "%s()  substream->runtime->rate %d  err:0x%X\n", __func__, substream->runtime->rate, err);
...
{
	printk_tmp("CALLED [alsa]: snd_card_TW68_hw_free\n");
   printk_n( "%s()\n", __func__);
	return 0;
}
...
{
	printk_tmp("CALLED [alsa]: snd_card_TW68_capture_close\n");
   printk_n( "%s()\n", __func__);
	return 0;
}
...
static int snd_card_TW68_capture_open(struct snd_pcm_substream * substream)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_capture_open\n");
	if (!card_TW68) {
		printk_n(KERN_ERR "BUG: TW68 can't find device struct."
	...
	printk_n( "%s()  substream %p  N:%d  %s rate %d  \n", __func__,
			     ...
			     snd_pcm_uframes_t count)
{
	...
	printk_tmp("CALLED [alsa]: snd_TW68_pcm_copy\n");
	...
	/*
	printk_n( "%s()  nId:%d;  count:%X %d  AP 0X%p ", __func__, nId, count, count, Audioptr);

	printk_n( "%s() dev 0x%p A 0x%X  k:%d  ss:%p  BF:%d  P:%d S:%d F:%d  \n", __func__, dev,  p_Audio.size, ss->number, ss,
		runtime->buffer_size, runtime->period_size, runtime->sample_bits, runtime->frame_bits  );
	*/
	avail = snd_pcm_capture_avail(runtime);
	//printk_n( "  card 0x%p,  TW68_pcm 0x%p  runtime BD 0%d  hw_ptr b0x%p, s0x%p, runtime->control->appl_ptr 0x%p,  Cap avail xfer_align 0x%X\n",
	...
	AudioVB = Audioptr + DMA_page - (avail* runtime->frame_bits/8);
		...
		{
			printk_n( " copy from io to user fail xxxx \n");
			return -EFAULT;
		}
	return 0;
}
...
static int snd_card_TW68_pcm_reg(snd_card_TW68_t *card_TW68, long idevice)
{
	...
	printk_tmp("CALLED [alsa]: snd_card_TW68_pcm_reg\n");
	printk_n(  "%s()\n", __func__);
...
int TW68_alsa_create(struct TW68_dev *dev)
{
	...
	int err;
	printk_tmp("CALLED [alsa]: TW68_alsa_create\n");
	...
	printk_n( "alsa: %s registered as card %s  card_TW68 %p  err :%d \n",card->longname, card->shortname, card_TW68, err);
	...
	if (err < 0)
	{	
		printk_n( "xxxxxx alsa: register TW68 card fail  :%d \n", err);
		return err;
	}
	printk_n( "alsa: %s registered as PCM card %s  :%d \n",card->longname, card->shortname, err);
	...
	printk_n( "xxxxxx alsa: %s registered as card %s  :%d \n",card->longname, card->shortname, err);
	///return err;
...
int TW68_alsa_free(struct TW68_dev *dev)
{
	...
	printk_tmp("CALLED [alsa]: TW68_alsa_free\n");
    if(dev->card) {
