...
#define dprintk_n(fmt, arg...)	if (core_debug) \
	printk_n(KERN_DEBUG "%s/core: " fmt, dev->name , ## arg)


void tw68v_set_framerate(struct TW68_dev *dev, u32 ch, u32 n)
{
	printk_tmp("CALLED [core]: tw68v_set_framerate\n");
	if (n >= 0  &&  n < 6)
	{
		if(ch >=0 && ch<8)
			reg_writel( DROP_FIELD_REG0+ ch,  video_framerate[n][0]);   // 30 FPS
		printk_n("%%%%    tw68v_set_framerate: ch Id %d   n:%d  %d FPS \n ", ch, n,  video_framerate[n][1]);
	}
}
...
{
	printk_tmp("CALLED [core]: dma_field_init\n");
	dma->kvirt = NULL;
	dma->dev = NULL;
	dma->n_pages = 0;
	dma->n_dma_pages = 0;
...
{
	printk_tmp("CALLED [core]: dma_field_free\n");
	if (dma->n_dma_pages) {
		     ...
		     struct pci_dev *dev, int direction)
{
	...
	printk_tmp("CALLED [core]: dma_field_alloc\n");
	...
	*/
	...
	/// printk_n( "dma_field_alloc: n_bytes %d   n_pages %d\n", n_bytes, dma->n_pages  );


	dma->kvirt = vmalloc_32(n_bytes);
	if (!dma->kvirt) {
		/// printk_n(KERN_ERR "dma_region_alloc: vmalloc_32() failed\n");
		goto err;
	}
	...
	if (!dma->sglist) {
		/// printk_n(KERN_ERR "dma_region_alloc: vmalloc(sglist) failed\n");
		goto err;
	}
	...
	if (dma->n_dma_pages == 0) {
		/// printk_n(KERN_ERR "dma_region_alloc: pci_map_sg() failed\n");
		goto err;
	}
...
{
	printk_tmp("CALLED [core]: TW68_buffer_pages\n");
...
int TW68_buffer_count(unsigned int size, unsigned int count)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_count\n");
	maxcount = 1024 / TW68_buffer_pages(size);
	if (count > maxcount)
		count = maxcount;


	 /// printk_n("TW68_buffer_count  size %d   maxcount %d / C %d \n",  size,	maxcount, count );
...
int TW68_buffer_startpage(struct TW68_buf *buf)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_startpage\n");
    pages = TW68_buffer_pages(buf->vb.bsize);
    n = buf->vb.i;
    pgn = pages * n;
    /// printk_n("TW68_buffer_startpage  = %d / %p %d %d =%d\n",
...
unsigned long TW68_buffer_base(struct TW68_buf *buf)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_base\n");
    // void* vbuf = videobuf_to_vmalloc(&buf->vb);

	base0  = TW68_buffer_startpage(buf) * 4096;
	base = base0 + dma->sglist[0].offset;

	/// printk_n("BBBBBB dma%x  vma%x  vbuf i%x  TW68_buffer_base  base0=%x / base =%x   offset %x\n",
...
int AudioDMA_PB_alloc(struct pci_dev *pci, struct TW68_pgtable *pt)
{
	...
	printk_tmp("CALLED [core]: AudioDMA_PB_alloc\n");
...
int videoDMA_pgtable_alloc(struct pci_dev *pci, struct TW68_pgtable *pt)
{
	...
	printk_tmp("CALLED [core]: videoDMA_pgtable_alloc\n");
    ...
    ///printk_n("  videoDMA_pgtable_alloc: cpu:0X%p  pt->size: 0x%x   BD:0X%x  \n", cpu, pt->size, (unsigned int)pt->cpu + pt->size );
    ...
    for ( clean = cpu; (unsigned int)clean < ((unsigned int)pt->cpu + pt->size); clean++ )
    {
           ...
           printk_n("---TW68_pgtable_alloc--- mem%0d   write CPU 0X%p  SIZE%d, clean 0X%p = 0X%x 0X%x   \n",
...
{
	printk_tmp("CALLED [core]: TW68_pgtable_free\n");
	if (NULL == pt->cpu)
		return;
...
void TW68_dma_free(struct videobuf_queue *q,struct TW68_buf *buf)
{
    ...
    /// printk_n("  $$$$ _dma_free :: dma->vmalloc 0x%X  vb->baddr %p \n", dma->vmalloc, buf->vb.baddr);
...
int TW68_buffer_queue(struct TW68_dev *dev,
			 ...
			 struct TW68_buf *buf)
{
	printk_tmp("CALLED [core]: TW68_buffer_queue\n");
	///printk_n(" %s: video buffer_queue %p    buf %p \n", __func__, q, buf);

	/*
	if (!list_empty(&q->queued)) {
		list_add_tail(&buf->vb.queue, &q->queued);
		buf->vb.state = VIDEOBUF_QUEUED;
		printk_n( "%s: [%p/%d] appended to queued\n",
				  ...
				  struct TW68_dmaqueue *q)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_requeue\n");

	printk_n( "%s: CALLED [core]\n", __func__);

	if (!list_empty(&q->active)) {
		buf = list_entry(q->active.next, struct TW68_buf, vb.queue);
		// printk_n( "%s: [%p/%d] restart dma\n", __func__,	buf, buf->vb.i);
	...
	for (;;) {
		...
		else 
		{
			list_move_tail(&buf->vb.queue, &q->active);
			buf->activate(dev, buf, NULL);
			/// printk_n( "%s: [%p/ %p] move to active\n", __func__, buf, buf->vb.i);
		} 
{
			//printk_n( "%s: no action taken\n", __func__);
			return 0;
		}
		prev = buf;
			   ...
			   unsigned int state)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_finish\n");
	if (q->dev != dev)	return;
	q->curr->vb.state = state;
	do_gettimeofday(&q->curr->vb.ts);

	/// printk_n("_buffer_finish ::buffer_finish %p -> NULL  k %p \n",q->curr, k);
			 ...
			 struct TW68_dmaqueue *q)
{
	...
	printk_tmp("CALLED [core]: TW68_buffer_next\n");
	BUG_ON(NULL != q->curr);
	...
	if (!list_empty(&q->queued)) {
		...
		///printk_n(KERN_INFO " $$$ buffer_activate  buf->vb.state = VIDEOBUF_ACTIVE; \n");
	       mod_timer( &q->timeout, jiffies+BUFFER_TIMEOUT);
	} else {
...
void Field_SG_Mapping(struct TW68_dev *dev, int  field_PB)  //    0 1
{
	...
	printk_tmp("CALLED [core]: Field_SG_Mapping\n");
	q = &dev->video_q;
	...
	if (!list_empty(&q->queued)) {
		...
		buf = list_entry(q->queued.next,struct TW68_buf,vb.queue);
		//
		printk_n("@@@@ locate buffer_next %p [prev=%p/next=%p]   &buf->vb.queue= %p \n",
    ...
    printk_n("Field_SG_Mapping  size %ld  Video Frame\n", buf->vb.size);
	...
	///printk_n("--_pgtable--nId 2 -- CPU ptr %p  P start address %p  &  dma_address %p \n", pt->cpu, ptr, pt->dma );
	...
	for (i = 0; i < dma->sglen; i++, list++)
		///for (p = 0; p * 4096 < list->length; p++, ptr++)
		{
		    ...
		    if ( ((nbytes + list->length) >= FieldSize) && ((nbytes + list->length) <=  (FieldSize + list->length)) )
			{
                ...
                if (remain >0)
                {
                    ...
                    printk_n("--_pgtable P/B switch ptr: %p  *(ptr-1)%x, *ptr%x  nbytes%d  FieldSize%d  remain%d   i%d\n", ptr, *(ptr-1), *ptr, nbytes, FieldSize, remain, i);
                }
				...
				printk_n("--_pgtable P/B i%d ->B  new ptr: %p    pt->cpu %p   nbytes%d  FieldSize%d  remain%d    \n", i, ptr, pt->cpu, nbytes, FieldSize, remain);
		        ...
		        ///printk_n("--_pgtable P/B switch ptr: %p  nbytes%d  FieldSize%d  remain%d \n", ptr, nbytes, FieldSize, remain);
		...
		/* nothing to do -- just stop DMA */
		printk_n("buffer_next %p   no more buffer \n",NULL);
	}
}
...
void Fixed_SG_Mapping(struct TW68_dev *dev, int nDMA_channel, int Frame_size)  //    0 1
{
	...
	printk_tmp("CALLED [core]: Fixed_SG_Mapping\n");
		///printk_n("@@@@ locate buffer_next %p [prev=%p/next=%p]   &buf->vb.queue= %p \n",
   ...
   /// printk_n("$$$$Fixed_SG_Mapping  nId %d  size %d  Video Frame %d\n", nId, FieldSize, Frame_size);
   /// printk_n("&&&&&&&&&&&&&&& Fill DMA table pt %p, length %d   startpage %d    L->length %d  \n",
	...
	printk_n("-??????????????????-_pgtable--nId %d,  pgn0 %d   pgn %d    m_NextFrameStartIdx: %d \n ", nId, pgn0, pgn,  m_NextFrameStartIdx);
	...
	for (i = 0; i < Field_P->n_pages; i++, sglist++)
	{
			...
			{
				/// printk_n("--#######@@@@@@@@@@@@@@@@@@@@_pgtable--nId %d     i=%d      remain=  %d   \n", nId, i, remain );
			}
...
void BFDMA_setup(struct TW68_dev *dev, int nDMA_channel, int H, int W)  //    Field0   P B    Field1  P B     WidthHightPitch
{
	...
	printk_tmp("CALLED [core]: BFDMA_setup\n");
   ...
   //  printk_n(KERN_INFO "DMA mode setup %s: %d PHASE_REF_CONFIG  dn 0x%lx    0x%lx  0x%lx  H%d W%d  \n",
...
int Field_Copy(struct TW68_dev *dev, int nDMA_channel, int field_PB)
{
	...
	/// printk_n(KERN_INFO " Field_Copy:  start 0000\n");
	...
	void *vbuf, *srcbuf;  // = videobuf_to_vmalloc(&buf->vb);
		...
		// fill P field half frame SG mapping entries
	printk_tmp("CALLED [core]: Field_Copy\n");
	...
	//  printk_n(KERN_INFO " nId%d   Field_Copy:  get DMA queue:%p,   curr %p\n", nId, q, q->curr);
	/*
	printk_n(KERN_INFO " Field_Copy:  get DMA queue list head:%p, \n", q->queue);
	printk_n(KERN_INFO " Field_Copy:  get DMA queue list head address:%p, \n", &q->queue);
	*/
	...
	{	
		...
		/// printk_n(KERN_INFO " Field_Copy:  start @@@@@@@@\n");   /// vbuf null
		...
		printk_n(KERN_INFO " Field_Copy:  Done %%%%%%%%%%\n");
    }
    else
	{	
		 printk_n( " Field_Copy:::::::::     list_empty  \n");
		return 0;
	}
       return 1;
}
...
int BF_Copy(struct TW68_dev *dev, int nDMA_channel, u32 Fn, u32 PB)
{
	...
	printk_tmp("CALLED [core]: BF_Copy\n");
		///printk_n("@@@@ locate buffer_next %p [prev=%p/next=%p]   &buf->vb.queue= %p \n",
		///	buf, q->queue.prev,q->queue.next,  &buf->vb.queue);
	...
	*/
	...
	{	
		...
		///printk_n(KERN_INFO " Field_Copy:  start @@@@@@@@   srcbuf:%X    vbuf:%X   H %d   W %d  P %d\n", srcbuf, vbuf, Hmax, Wmax, pitch);   /// vbuf null
...
#endif
		printk_n(KERN_INFO " BField_Copy:  Done %%%%%%%%%%\n");
    }
    else
	{	
		printk_n( " Block [][] Field_Copy:::::::::     list_empty  \n");
		return 0;
	}
       return 1;
}
...
int  QF_Field_Copy(struct TW68_dev *dev, int nDMA_channel, u32 Fn, u32 PB)
{
	...
	void *vbuf, *srcbuf;  // = videobuf_to_vmalloc(&buf->vb);
	printk_tmp("CALLED [core]: QF_Field_Copy\n");
...
*/
	...
	{	
		...
		printk_n(KERN_INFO " QField_Copy:  Done %%%%%%%%%%\n");
       }
      else
	{	
		return 0;
...
void DecoderResize(struct TW68_dev *dev, int nId, int nHeight, int nWidth)
{
	...
	printk_tmp("CALLED [core]: DecoderResize\n");
	if(nId >=8 ) {
		/// printk_n( "DecoderResize() error: nId:%d,Width=%d,Height=%d\n", nId, nWidth, nHeight);
		return;
	}
	/// printk_n( "DecoderResize() ::::: nId:%d,Width=%d,Height=%d\n", nId, nWidth, nHeight);
	...
	nH = nW = nHW;
	printk_n( "DecoderResize() :: nId:%d,  H:%d  W:%d   HW %X  HDELAY0 %X\n", nId, nHeight, nWidth, nHW, nVal);   /// read default  0x0a;
...
*/
	...
	//// printk_n( "DecoderResize() ?????????????????:: nId:%d,    nAddr%X  nReg:%X\n", nId, nAddr, nReg);
...
void resync(unsigned long data)
{
	...
	{
		///printk_n(" resync _reset_ sync time now%lX - errlog %lX \n", now, dev->errlog[0]);
		return;
	}
...
u64 GetDelay(struct TW68_dev *dev, int eno)
{
	...
	printk_tmp("CALLED [core]: GetDelay\n");
	last =0;
	pause = 40;  // 20  30  50
...
*/
	...
	///printk_n( " last 0X%x   jiffies 0X%x ", last, jiffies);
	now = jiffies;
	if (last > now)
		delay = last;
	else
...
void TW68_buffer_timeout(unsigned long data)
{
   u32 dwRegE, dwRegF;
	...
	printk_tmp("CALLED [core]: TW68_buffer_timeout\n");
	if (q->curr) {
		
	dwRegE = reg_readl(DMA_CHANNEL_ENABLE);
	dwRegF = reg_readl(DMA_CMD);

	printk_n(" TW68_buffer_timeout ????????  DMA %d  || 0x%X  ||0X%X     timeout on dma queue %p\n", nId,  dwRegE, dwRegF, q->curr);
...
int TW68_set_dmabits(struct TW68_dev *dev,  unsigned int DMA_nCH)
{
	...
	printk_tmp("CALLED [core]: TW68_set_dmabits\n");
	nId = DMA_nCH;   ////
	...
	dwRegF = reg_readl(DMA_CMD);
		...
		/// printk_n(" _dmabits _set_dmabits : DMA_INT_STATUS 0X%X  DMA_INT_ERROR%X  DMA_PB_STATUS%X  DMA_CHANNEL_ENABLE %x  DMA_CMD %X \n", dwRegST, dwRegER, dwRegPB, dwRegE, dwRegF);
	...
	reg_writel(DMA_CMD, dwRegF);
		///dwRegF = reg_readl(DMA_CMD);
		/// printk_n(" set_dmabits ?????????????? _set_dmabits : DMA_CHANNEL_ENABLE %x  DMA_CMD %X \n", dwRegE, dwRegF);

	/// printk_n(" TW68_set_dmabits _set_dmabits : DMA_INT_STATUS%X  DMA_INT_ERROR%X  DMA_PB_STATUS%X  DMA_CHANNEL_ENABLE %x  DMA_CMD %X \n", dwRegST, dwRegER, dwRegPB, dwRegE, dwRegF);
	return 0;
}
...
int	stop_video_DMA(struct TW68_dev *dev, unsigned int DMA_nCH)
{
	...
	printk_tmp("CALLED [core]: stop_video_DMA\n");
	nId = DMA_nCH;  ///2;
    ...
    /// printk_n(KERN_INFO " -------------stop_video_DMA   DMA_CHANNEL_ENABLE 0x%X,  DMA_CMD 0x%X,  DMA_INT_STATUS 0x%08X \n",
...
{
		u32	regDW, dwReg;
		printk_tmp("CALLED [core]: VideoDecoderDetect\n");
		...
		if (( regDW &1)) //&& (!(dwReg & 0x80)))   ///skip the detection glitch     //detect properly
		{
			...
			// VDelay
			printk_n("50HZ VideoStandardDetect DMA_nCH %d  regDW 0x%x  dwReg%d \n", DMA_nCH, regDW, dwReg );
			return 50;
		}
		else
		{
			printk_n("60HZ VideoStandardDetect DMA_nCH %d  regDW 0x%x  dwReg%d \n", DMA_nCH, regDW, dwReg );
			return 60;
...
void video_tasklet(unsigned long device)
{
	...
	printk_tmp("CALLED [core]: video_tasklet\n");
...
{	
	...
	static int INT1st = 1;
	static u32 lastPB = 0;
	printk_tmp("CALLED [core]: TW68_irq\n");
	...
	///////////////////////////////////////////////////////////////////////////
		if 	(dev->videoDMA_ID != dwRegE)
		{
			INT1st++;
			if (INT1st <10)
			printk_n(KERN_INFO " -------------Check DMA   DMA_CHANNEL_ENABLE 0x%X,  DMA_CMD 0x%X,  DMA_INT_STATUS 0x%08X   videoDMA_ID 0x%X\n",
	...
	for (audio_ch =0; audio_ch < MAX_NUM_DATA_DMA; audio_ch++)
	{
		...
		/// printk_n("---Audio DMA 0x19-0x29 config --- CH%02d    *dmaP 0X%x  *dmaB 0X%x  dword 0X%x 0X%x   \n",
	...
	*/
    ...
    if((dwRegER &0xFF000000) && dev->video_DMA_1st_started && dev->err_times<9)
	{
		...
		dev->err_times++;
		printk_n("DeviceInterrupt: 1st startup err_times:%d ## dma_status (err) =0x%x   dwRegVP (video parser)=0X%x   int_status 0x%x   dwRegE 0X%x \n", dev->err_times, dwRegER, dwRegVP, dwRegST, dwRegE);
	}
	else
	{
...
//		  dwRegF = reg_readl(DMA_CHANNEL_ENABLE);
		  ...
		  spin_unlock_irqrestore(&dev->slock, flags);
		   ...
		   printk_n("DeviceInterrupt: errors  ## dma_status  0x%X   (err) =0x%X   dwRegVP (video parser)=0X%x   int_status 0x%x  # dwRegE 0X%x dwRegF 0X%x \n",
	  ...
	  {
		 // Normal interrupt:
	     // printk_n("  Normal interrupt:  ++++ ## dma_status 0x%X   FIFO =0x%X   (video parser)=0X%x   int_status 0x%x  PB 0x%x  # dwRegE 0X%x dwRegF 0X%x \n",
			  ...
			  if ((dwRegST & (0xFF)) && (!(dwRegER >>16)))
			  {
				  ...
				  for(k=0; k<8; k++)
				  {
					if ((dwRegST & dev->videoDMA_ID) & ( 1 << k) )     /// exclude  inactive dev
					{
							...
							/// printk_n(" IRQ DMA normal ist time  k:%d  Reg8b 0x%x: PB 0x%x  FieldPB 0X%X DMA_CHANNEL_ENABLE %x  DMA_CMD %X \n",
    ...
    if (!dwRegER && !dwRegST)  // skip the  interrupt  conflicts
	{
		...
		*/
        ...
        handled = 0;
		/// printk_n("---+++++++++ skip IRQ: DMA_INT_STATUS 0X%X  DMA_INT_ERROR%X  DMA_PB_STATUS%X  DMA_CHANNEL_ENABLE %x  DMA_CMD %X  k:%x\n", dwRegST, dwRegER, dwRegPB, dwRegE, dwRegF, k);
	}
...
static int TW68_hwinit1(struct TW68_dev *dev)
{
	...
	int audio_ch;
	u32 dmaP, dmaB;
	printk_tmp("CALLED [core]: TW68_hwinit1\n");

	printk_n(" TW6869 hwinit1 \n");
	mutex_init(&dev->lock);
    ...
    pci_read_config_dword( dev->pci, 0x78, &regDW);
    //  printk_n(KERN_INFO "%s: CFG[0x78]   0x%lx\n", dev->name,  regDW );

    pci_read_config_dword( dev->pci, 0x730, &regDW);
    //  printk_n(KERN_INFO "%s: CFG[0x730]   0x%lx\n", dev->name,  regDW );

    pci_read_config_dword( dev->pci, 0x734, &regDW);
    //  printk_n(KERN_INFO "%s: CFG[0x734]   0x%lx\n", dev->name,  regDW );

    pci_read_config_dword( dev->pci, 0x738, &regDW);
    //  printk_n(KERN_INFO "%s: CFG[0x738]   0x%lx\n", dev->name,  regDW );
    ...
    //printk_n(KERN_INFO "%s: PCI_CFG[Posted 0x730]= 0x%lx\n", dev->name,  regDW );

	//Trasnmit Non-Posted FC credit Status
	reg_writel(EP_REG_ADDR, 0x734);   //
	regDW = reg_readl( EP_REG_DATA );
    //printk_n(KERN_INFO "%s: PCI_CFG[Non-Posted 0x734]= 0x%lx\n", dev->name,  regDW );

	//CPL FC credit Status
	reg_writel(EP_REG_ADDR, 0x738);   //
	regDW = reg_readl( EP_REG_DATA );
    //printk_n(KERN_INFO "%s: PCI_CFG[CPL 0x738]= 0x%lx\n", dev->name,  regDW );


	/////////////////////////////////////////////////////////////////////////////////////////////

    regDW = reg_readl( (SYS_SOFT_RST) );
    /// printk_n(KERN_INFO "HWinit %s: SYS_SOFT_RST  0x%lx    \n",  dev->name, regDW );
    ///regDW = tw_readl( SYS_SOFT_RST );
    ///printk_n(KERN_INFO "DMA %s: SYS_SOFT_RST  0x%lx    \n",  dev->name, regDW );

	reg_writel((SYS_SOFT_RST), 0x01);   //??? 01   09
	reg_writel((SYS_SOFT_RST), 0x0F);
    regDW = reg_readl( SYS_SOFT_RST );
    ///printk_n(KERN_INFO " After software reset DMA %s: SYS_SOFT_RST  0x%lx    \n",  dev->name, regDW );

    regDW = reg_readl( PHASE_REF_CONFIG );
    ///printk_n(KERN_INFO "HWinit %s: PHASE_REF_CONFIG  0x%lx    \n",  dev->name, regDW );
	regDW = 0x1518;
...
#endif
		{
			/// printk_n(KERN_ERR,  "Failed to allocate dma buffer");
...
//	m_nVideoFormat = VIDEO_FORMAT_RGB565;
   ...
   for (k = 0; k <MAX_NUM_SG_DMA; k++)
   {
		...
		m_dwCHConfig                   = ( m_StartIdx&0x3FF)			|    // 10 bits
										 ...
										 ((m_nVideoFormat&7)<<20)		|
										 ((m_bHorizontalDecimate=0&1)<<23)|
										 ((m_bVerticalDecimate=0&1)<<24)	|
		...
		/// printk_n(" ********#### buffer_setup%d::  m_StartIdx 0X%x  0x%X  dwReg: 0x%X  m_dwCHConfig 0x%X  \n", k, m_StartIdx, pgn,  m_dwCHConfig, dwReg );
...
// setup audio DMA
	for (audio_ch =0; audio_ch < MAX_NUM_DATA_DMA; audio_ch++)
	{
		...
		/// printk_n("---Audio DMA 0x19-0x29 config --- CH%02d   write dmaP 0X%p  dmaB 0X%p  read 0X%x 0X%x   \n",
	...
	{
		*(Audioptr +k)= 0xF0;
		///printk_n(" 0x%x ", (*(Audioptr +k)));
	}
	printk_n(" \n");
	*/
    ///printk_n(KERN_INFO " Mmio %s: CFG[0x78]  cpu 0x%x    dma 0x%x \n", dev->name,  (unsigned int)dev->m_Page0.cpu, dev->m_Page0.dma );

    regDW = reg_readl( (DMA_PAGE_TABLE0_ADDR) );
    printk_n(KERN_INFO "DMA %s: DMA_PAGE_TABLE0_ADDR  0x%x    \n",  dev->name, regDW );
    regDW = reg_readl( (DMA_PAGE_TABLE1_ADDR) );
    printk_n(KERN_INFO "DMA %s: DMA_PAGE_TABLE1_ADDR  0x%x    \n",  dev->name, regDW );
    ...
    printk_n(KERN_INFO "DMA %s: tw DMA_PAGE_TABLE0_ADDR  0x%x    \n",  dev->name, regDW );
    regDW = tw_readl( (DMA_PAGE_TABLE1_ADDR) );
    printk_n(KERN_INFO "DMA %s: tw DMA_PAGE_TABLE1_ADDR  0x%x    \n",  dev->name, regDW );
	*/

	reg_writel(AVSRST,0x3F);        // u32
    regDW = reg_readl(AVSRST);
	printk_n(KERN_INFO "DMA %s: tw AVSRST _u8 %x :: 0x%x    \n",  dev->name, (AVSRST<<2), regDW );

	reg_writel(DMA_CMD, 0);          // u32
    regDW = reg_readl( DMA_CMD );
	printk_n(KERN_INFO "DMA %s: tw DMA_CMD _u8 %x :: 0x%x    \n",  dev->name, (DMA_CMD<<2), regDW );

	reg_writel(DMA_CHANNEL_ENABLE,0);
    regDW = reg_readl( DMA_CHANNEL_ENABLE );
	printk_n(KERN_INFO "DMA %s: tw DMA_CHANNEL_ENABLE %x :: 0x%x    \n",  dev->name, DMA_CHANNEL_ENABLE, regDW );
	...
	printk_n(KERN_INFO " read tw68 VDELAY0 %x :: 0x%x    \n",  VDELAY0, regDW );

    regDW = reg_readl( MISC_CONTROL2 );
	printk_n(KERN_INFO " read tw68 MISC_CONTROL2 %x :: 0x%x    \n",  MISC_CONTROL2, regDW );
	...
	/// printk_n("2864 init CSR_REG 0x%x]=  I2C 2864   val1:0X%x  %x\n", CSR_REG,  val1 );
	...
	/// printk_n("2864[CLKOCTRL 0x%x]=  I2C 2864  val0:0X%x   val1:0X%x   val2:0X%x\n", CLKOCTRL, val0, val1, val2 );
...
{
	printk_tmp("CALLED [core]: TW68_hwfini\n");
	dprintk_n("hwfini\n");
...
static int vdev_init (struct TW68_dev *dev, struct video_device *template,  char *type)
{
    ...
    int k=1;
	int err0;
	printk_tmp("CALLED [core]: vdev_init\n");
    ...
    for ( k =1; k<9; k++ )     // dev0  QF muxer  dev 1 ~ 4
    {
        ...
        if (NULL == vfdev[k])
        {
            printk_n(KERN_WARNING "Null  vfdev %d **** \n\n", k);
            return k;
...
{
    int k;
	printk_tmp("CALLED [core]: TW68_unregister_video\n");
    for ( k =1; k<9; k++ )   /// 0 + 4
    {
	...
	if (dev->video_device[k]) 
		if (-1 != dev->video_device[k]->minor)
		{
			...
			printk_n(KERN_INFO "video_unregister_device(dev->video_dev %d  \n", k );
		}
				     ...
				     const struct pci_device_id *pci_id)
{
	struct TW68_dev *dev;
	int err, err0;
	printk_tmp("CALLED [core]: TW68_initdev\n");

	printk_n(KERN_INFO "PCI register init CALLED [core]\n");
	...
	printk_n(KERN_INFO " %s TW68_devcount: %d \n", dev->name, TW68_devcount );

	/* pci quirks */
	if (pci_pci_problems) {
		if (pci_pci_problems & PCIPCI_TRITON)
			printk_n(KERN_INFO "%s: quirk: PCIPCI_TRITON\n", dev->name);
		if (pci_pci_problems & PCIPCI_NATOMA)
			printk_n(KERN_INFO "%s: quirk: PCIPCI_NATOMA\n", dev->name);
		if (pci_pci_problems & PCIPCI_VIAETBF)
			printk_n(KERN_INFO "%s: quirk: PCIPCI_VIAETBF\n", dev->name);
		if (pci_pci_problems & PCIPCI_VSFX)
			printk_n(KERN_INFO "%s: quirk: PCIPCI_VSFX\n",dev->name);
#ifdef PCIPCI_ALIMAGIK
		if (pci_pci_problems & PCIPCI_ALIMAGIK) {
			printk_n(KERN_INFO "%s: quirk: PCIPCI_ALIMAGIK -- latency fixup\n",
			       dev->name);
			latency = 0x0A;
		}
#endif
		if (pci_pci_problems & (PCIPCI_FAIL|PCIAGP_FAIL)) {
			printk_n(KERN_INFO "%s: quirk: this driver and your "
					"chipset may not work together"
					" in overlay mode.\n",dev->name);
			if (!TW68_no_overlay) {
				printk_n(KERN_INFO "%s: quirk: overlay "
...
/*
	if (UNSET != latency) {
		printk_n(KERN_INFO "%s: setting pci latency timer to %d\n",
		       dev->name,latency);
...
*/
	...
	printk_n(KERN_INFO "%s: found at %s, rev: %d, irq: %d, "
	...
	if (!pci_dma_supported(pci_dev, DMA_BIT_MASK(32))) {
		printk_n("%s: Oops: no 32bit PCI DMA ???\n",dev->name);
		err = -EIO;
		goto fail1;
	}
	else
        printk_n("%s: Hi: 32bit PCI DMA supported \n",dev->name);


    dev->board = 1;
    printk_n(KERN_INFO "%s: subsystem: %04x:%04x, board: %s [card=%d,%s]\n",
	...
	if (!request_mem_region(pci_resource_start(pci_dev,0),
				...
				dev->name)) {
		err = -EBUSY;
		printk_n(KERN_ERR "%s: can't get MMIO memory @ 0x%llx\n",
	...
	if (NULL == dev->lmmio) {
		err = -EIO;
		printk_n(KERN_ERR "%s: can't ioremap() MMIO memory\n",
		       dev->name);
		goto fail2;
	}

   	//printk_n(KERN_INFO "	TW6869 PCI_BAR0 mapped registers: phy: 0x%X   dev->lmmio 0X%x  dev->bmmio 0X%x   length: %x \n",
	...
	/* get irq */
    printk_n( "TW68_initdev   %s: request IRQ %d\n",     dev->name,pci_dev->irq);
	 ...
	 if (err < 0) {
		printk_n(KERN_ERR "%s: can't get IRQ %d\n",
		       dev->name,pci_dev->irq);
		goto fail3;
	 }
	...
	printk_n(KERN_ERR "Adding  TW686v_devlist %p\n", &TW686v_devlist);
	list_add_tail(&dev->devlist, &TW686v_devlist);
	////add current TW68_dev device structure node

	/* register v4l devices */
	if (TW68_no_overlay > 0)
		printk_n(KERN_INFO "%s: Overlay support disabled.\n", dev->name);
    else
       	printk_n(KERN_INFO "%s: Overlay supported %d .\n", dev->name, TW68_no_overlay);

   err0 = vdev_init(dev, &TW68_video_template,"video");    

	if (err0 < 0) {
		printk_n(KERN_INFO "%s: can't register video device\n",
		       dev->name);
		goto fail4;
	}

    TW68_devcount++;
    printk_n(KERN_INFO "%s: registered PCI device %d [v4l2]:%d  err: |%d| \n",
...
static void TW68_finidev(struct pci_dev *pci_dev)
{
	...
	printk_tmp("CALLED [core]: TW68_finidev\n");
	printk_n(KERN_INFO "%s: Starting unregister video device %d\n",
	       dev->name, dev->video_device[1]->num);


	printk_n(KERN_INFO " /* shutdown hardware */ dev 0x%p \n" , dev);
	...
	printk_n(KERN_INFO " list_del(&dev->devlist) \n ");
	...
	for ( k =0; k<8; k++)
	{
	 ...
	 TW68_unregister_video(dev);
	 TW68_alsa_free(dev);
	 printk_n(KERN_INFO " TW68_unregister_video(dev); \n ");

	//v4l2_device_unregister(&dev->v4l2_dev);
	printk_n(KERN_INFO " unregistered v4l2_dev device  %d %d %d\n",
...
{
	printk_tmp("CALLED [core]: TW68_init\n");
	INIT_LIST_HEAD(&TW686v_devlist);
	printk_n(KERN_INFO "TW68_: v4l2 driver version %d.%d.%d loaded\n",
			TW68_VERSION_CODE >>16, (TW68_VERSION_CODE >>8)&0xFF, TW68_VERSION_CODE &0xFF);
...
{
	printk_tmp("CALLED [core]: TW68_fini\n");
	pci_unregister_driver(&TW68_pci_driver);
	printk_n(KERN_INFO "TW68_: v4l2 driver version %d.%d.%d removed\n",
			TW68_VERSION_CODE >>16, (TW68_VERSION_CODE >>8)&0xFF, TW68_VERSION_CODE &0xFF);
